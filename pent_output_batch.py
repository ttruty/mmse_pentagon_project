...
cwd = os.getcwd()
model = os.path.join(cwd, 'training_mmse_pentagons')
model_path = os.path.join(model, "models", "svm.model")

# working_path = r'C:\Users\KinectProcessing\Desktop\sample_output'
# for _,_,files in os.walk(working_path):
#     for i in files:
#         naming_file.parse_filename(i)
...
working_path = r'C:\Users\KinectProcessing\Desktop\sample_output_no-corner-limit'
for _,_,files in os.walk(working_path):
    for file in files:
        #naming_file.parse_filename(file)
# path = tkinter.filedialog.askopenfilename()
        path = os.path.join(working_path, file)
        print(path)
        cropped, im, found_dets = find_contours(path, 20, .2, 2, 0)
        ...
        goodcorners = cv2.goodFeaturesToTrack(thin_bw, 500, .05, 5)
        cornerlist = []
        ...
        # Output Textfiles
        with open(path + "_SampleOutput" + ".txt", "w") as text_file:
        ...
        with open(path + "_cornerlist_output.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(cornerlist)

        with open(path + "_linelist_output.csv", "w", newline="") as f:
            writer = csv.writer(f)
            for i in lines:
                writer.writerows([i[0],i[1]])

        with open(path + "_gapslist_output.csv", "w", newline="") as f:
            writer = csv.writer(f)
            for i in gaps:
                writer.writerows([i[0],i[1]])
